...
fn main() -> Result<()>{
    ...
    let (mut player_1_stream, mut player_2_stream) = accept_clients("127.0.0.1:7878").unwrap();
    ...
    loop {
        ...
        if std::str::from_utf8(&p1_buffer) != Ok("Quit") && std::str::from_utf8(&p2_buffer) != Ok("Quit") {
...
fn accept_clients(server_addr: &str) -> Result<(TcpStream, TcpStream)> {
    ...
    for stream in listener.incoming() {
        ...
        if !connection_1_set {
            ...
            //println!("Address '{}' is connected as player 1", connection_1.unwrap().peer_addr()?);
            ...
            //println!("Address '{}' is connected as player 2", connection_2.unwrap().peer_addr()?);
